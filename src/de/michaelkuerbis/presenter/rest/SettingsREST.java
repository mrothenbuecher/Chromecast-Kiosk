...
public class SettingsREST {
	...
	@POST
	@Path("/add/{ip}/{name}")
	public Response addChromecast(@PathParam("ip") String ip, @PathParam("name") String name){
		...
		if(ip != null && !ip.isEmpty() && name != null && !name.isEmpty()){
			...
			for(CastConnection con : SettingsServlet.getConnections()){
				if(con.getIp().equals(ip)){
					alreadyDefined = true;
					obj.put("error", "chromecast already defined with name: "+ con.getName());
					break;
				}
			}
			if(!alreadyDefined){
				CastConnection con = new CastConnection(ip, name);
				Vector<CastConnection> vec = SettingsServlet.getConnections();
				vec.add(con);
				SettingsServlet.setConnections(vec);
				if(SettingsServlet.saveSettings(this.webRequest.getServletContext())){
					obj.put("ok", "");
				}else{
